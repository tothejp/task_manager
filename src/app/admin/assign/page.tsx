...
export default async function AdminAssignPage({
  ...
  const [
    ...
    supabase
      ...
      .eq("date", date)
      .neq("status", "EMPTY"),
    // 휴가 등록으로 공백 처리된 배정 (PRD 3.7) — 재배정 대상임을 안내하기 위해 별도 조회
    supabase.from("assignments").select("task_id, member_id").eq("date", date).eq("status", "EMPTY"),
  ]);
  ...
  for (const a of assignmentsRes.data ?? []) {
    ...
    if (!task) continue; // 방어적 처리: 같은 날짜 과업 목록에 없으면 무시
