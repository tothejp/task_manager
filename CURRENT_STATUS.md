...
## 현재 단계: 중대현황판 UX 정리 + 스킬→능력 용어 변경 + 프로젝트 소개 발표자료(`/present`) 추가 완료. DB 마이그레이션 2건 미실행 상태로 남아있음(계급 기본값, 능력 태그 전역 유니크), 당장 막힌 작업은 없음

---

### 여섯 번째 라운드 (2026-07-31)
- **프로젝트 소개 발표자료를 `/present` 공개 페이지로 추가**: 개요/배경/목적/주요 기능/기능 상세(팀원 모바일·관리자 중대현황판·능력/자동배정/공정성)/배정 정책/기술 스택/라이브 데모 전환/로드맵/마무리로 구성된 12섹션 스크롤 프레젠테이션. 로그인 없이 접속 가능(`(app)`/`(auth)` 라우트 그룹 밖이라 인증 체크 없음). 처음엔 Claude 아티팩트로 만들었다가, 사용자 요청으로 저장소에 커밋해 앱 자체 라우트로 옮김.
  - **구현 방식**: 앱 전역 Tailwind/디자인 토큰과 스타일이 부딪히지 않도록, 완전히 독립된 HTML 문서(자체 CSS 변수/폰트/스크롤 스냅 로직)를 문자열로 만들어([app/present/deck.ts](app/present/deck.ts)) `<iframe srcDoc={deckHtml}>`로 그대로 띄우는 방식([app/present/page.tsx](app/present/page.tsx)) 채택.
  - **실제 화면 스크린샷 대신 목업 사용**: 이 세션 환경에서 배포 사이트 스크린샷 캡처가 안 돼서, 실제 컴포넌트 구조(계급 열이 있는 인원표, 배정완료/미배정 배지, 요구인원 제한없음 표시 등)를 코드 기준으로 그대로 재현한 CSS 목업을 넣음 — 실제 화면과 문구가 어긋나지 않음.
  - **검증 한계**: 배포 후 슬라이드 12개·네비게이션 점·달력 목업 등 정적 렌더링은 확인했지만, 스크롤 기반 슬라이드 전환(키보드/버튼 네비게이션)은 이 세션의 자동화 브라우저가 탭을 백그라운드로 인식해(`document.hidden=true`) 직접 확인하지 못함 — 표준 `scroll-snap`+`IntersectionObserver`라 실제 브라우저에서는 동작할 것으로 보이나, **실사용 시 화살표 키/우측 점 네비게이션이 정상 작동하는지 확인 필요**.

### 다섯 번째 라운드 (2026-07-31)
- **중대현황판 — "미완료 과업" 섹션 제거**: 사용자 판단으로 불필요해져 섹션과 관련 쿼리(과거 미완료 배정 조회) 전부 제거([app/(app)/admin/page.tsx](app/(app)/admin/page.tsx)).
- **"충원" → "배정" 용어 통일**: 배정 보드/조회 전용 목록의 "충원 완료"/"미충원 N" 표시를 "배정 완료"/"미배정 N"으로 변경([components/admin/AssignmentBoard.tsx](components/admin/AssignmentBoard.tsx)).
- **요구인원 "제한없음" 추가**: 요구인원 드롭다운 맨 위에 "제한없음" 옵션 추가(내부적으로 0을 특수값으로 사용, [lib/task-headcount.ts](lib/task-headcount.ts)). 제한없음 과업은 배정 보드에서 "빈 슬롯"/충원 배지 없이 항상 배정 가능하도록 처리.
  - **배포 후 발견한 버그**: 클라이언트 쪽 배정 가능 여부 체크(`AssignmentBoard.tsx`)는 고쳤지만, 서버 액션의 재검증 로직(`insertAssignmentIfValid`, [app/(app)/admin/assign/actions.ts](app/(app)/admin/assign/actions.ts))이 여전히 `count >= required_headcount`로 비교하고 있어 제한없음(0) 과업엔 배정이 아예 막히는 버그가 있었음. 실제 D&D로 배포 환경에서 테스트하다 발견해서 바로 수정/재배포함 — **자동화 테스트로 검증할 때 클라이언트 쪽 로직만 보고 넘어가지 말고 서버 액션의 방어적 재검증 로직도 같이 확인할 것.**
- **"스킬" → "능력" 용어 변경**: 사용자 노출 텍스트/주석 전반에서 "스킬"을 "능력"으로 교체(과업 관리/조직원 관리/배정 보드 등). 코드 식별자(`skillTags`, `SkillManagement`, `skill_tags` 테이블/컬럼 등)는 CLAUDE.md 규칙대로 영어 네이밍 그대로 유지 — DB 스키마 변경 없음.
- **능력 태그 전역 유니크 마이그레이션 추가**: `(team_id, name)` 유니크를 `name` 단독 전역 유니크로 교체하는 스크립트 작성([supabase/skill_tags_global_unique.sql](supabase/skill_tags_global_unique.sql), 기존 중복 태그 자동 병합 포함) — **아직 미실행**.
...
- [x] **중대 현황판 — `/admin`**: 가용인원 현황(전체/가용 인원, 이름/계급/상태 테이블 — 계급 미지정 시 기본값 '이병') + **과업 배정(Drag & Drop)까지 이 페이지 하나로 통합**. 완료율 섹션·능력 요약 카드·미완료 과업 섹션은 사용자 요청으로 제거됨. 날짜 선택은 공용 `DatePickerField`(클릭 시 모달로 월 달력) 사용
- [x] 과업 관리 — `/admin/tasks`: 생성/**수정**(모달 폼)/삭제. 시간 단위 관리는 제거하고 날짜 단위로만 관리(내부적으로 `start_time=00:00`/`end_time=23:59` 고정값 사용 — 같은 날 여러 과업에 한 명을 배정할 수 없는 트레이드오프 있음, 확인 후 진행). 요구인원 드롭다운 맨 위에 "제한없음" 옵션 있음
- [x] D&D 과업 배정 — 기존 `/admin/assign` 페이지는 삭제되고 `/admin`(중대 현황판)에 통합됨. 사이드바에서 "과업 배정" 메뉴도 제거. 시간 중복 하드 차단, 필수 능력 미보유는 경고 모달(하드 차단 아님) + `skill_override` 기록. 요구인원 "제한없음" 과업은 인원 수 제한 없이 배정 가능
...
- [x] **조직원 관리 — `/admin/organization`** (구 "팀원 승인" 탭 대체): 세 영역으로 구성
  ...
  3. 능력 태그 관리 — 태그 생성 + **Drag & Drop으로 조직원에게 부여**(능력 칩을 조직원 위로 드래그), 배지 클릭으로 회수. 삭제는 "🗑 제거" 드롭존 위로 드래그(확인창 거침) — 더블클릭/롱프레스 삭제는 부여용 D&D와 충돌해서 제거됨. 태그 이름은 팀 구분 없이 전역 유일해야 함(마이그레이션 미실행 — 아래 SQL 목록 참고)
...
- [x] 프로젝트 소개 발표자료 — `/present`, 로그인 불필요한 공개 페이지. 개요/배경/목적/기능 상세/배정 정책/기술 스택/로드맵을 담은 스크롤 프레젠테이션([app/present/page.tsx](app/present/page.tsx), [app/present/deck.ts](app/present/deck.ts))
...
## 다음 단계
0. 미실행 SQL 스크립트 2건 실행 필요 (급하지 않음 — 실행 전까지 앱 코드가 안전하게 동작):
   - `supabase/member_rank_default.sql` — 계급 미지정 인원을 DB에 '이병'으로 채우고 컬럼 기본값/NOT NULL 설정
   - `supabase/skill_tags_global_unique.sql` — 능력 태그 이름을 전역 유일하게 변경(기존 중복 자동 병합)
1. 실제 사용자 기기(모바일/평소 브라우저, 화면이 보이는 상태)에서 과업 수정·사이드바 이동·능력 부여 체감 속도가 실제로 개선됐는지 확인 — 자동화 테스트로는 액션당 1.1~1.6초로 측정됨(정확한 방법으로 재측정한 값), 개선 전 수치는 측정 오류로 신뢰 불가해 정확한 비교는 어려움
2. 능력 태그가 팀 구분 없이 전역으로 보이는지, 다른 팀 관리자가 만든 능력도 D&D 가능한지 실사용 확인 (RLS 스크립트 실행 완료)
3. 능력 태그 삭제 — "🗑 제거" 드롭존으로 드래그하는 방식이 모바일 터치에서도 자연스러운지 실사용 확인 (배포 환경에서 키보드 D&D로 로직 자체는 검증했으나 실제 터치 드래그 체감은 미확인)
4. 모바일에서 팀원 사이드바 아이콘/`/me` 이동 확인, 휴가 등록(특히 여러 날짜 범위/매주 반복) 저장 속도 및 "저장완료" 표시 실사용 확인
5. 중대 현황판 — 날짜 선택 팝오버 위치/너비, 전체·가용 인원 접기 펼치기(계급 열 포함), 배정 완료/미배정 표시, 요구인원 제한없음 과업 실사용 확인
6. 과업 관리 — 과업명/설명 인라인 수정, 요구인원 드롭다운(제한없음 포함), "새 과업 만들기" 모달 실사용 확인
7. 공정성 지표 — 사용자가 "차후 기능 개선 예정"이라고 밝힘, 다음 요청 대기
8. `/present` 발표자료 — 실제 브라우저에서 화살표 키/우측 점 클릭으로 슬라이드가 정상 전환되는지 확인 (자동화 세션의 백그라운드 탭 제약으로 이 부분만 미검증 상태)
...
- **요구인원 0 = "제한없음"**: `lib/task-headcount.ts`의 특수값(2026-07-31부터). DB 스키마 변경 없이 기존 `required_headcount` 컬럼을 재사용 — 화면 표시(`formatHeadcount`)뿐 아니라 배정 가능 여부를 판단하는 클라이언트/서버 로직 양쪽에서 모두 이 값을 특별 취급해야 한다. 자동배정(`recommendAssignments`)은 이런 과업을 자동으로 건너뛴다(목표 인원수가 없어 "몇 명을 채워야 하는지" 계산 불가 — 의도된 동작, 수동 배정만 가능).
- **"스킬" → "능력"**: 사용자 노출 텍스트는 "능력"으로 통일(2026-07-31부터). 코드 식별자/DB 컬럼·테이블명(`skill_tags`, `skillTags` 등)은 영어 네이밍 그대로 유지 — 새 코드 작성 시 변수명은 기존처럼 `skill*`을 쓰고, 사용자에게 보이는 문구만 "능력"으로 쓸 것.
- **`/present`는 인증·팀 데이터와 무관한 독립 페이지**: `(app)`/`(auth)` 라우트 그룹 밖에 둬서 로그인 체크를 받지 않음. 앱의 Tailwind/디자인 토큰과 충돌하지 않도록 자체 CSS 변수를 가진 완결된 HTML 문서를 `app/present/deck.ts`에 문자열로 두고 `iframe srcDoc`으로 렌더링 — 새 발표자료/랜딩류 페이지를 추가할 때 같은 패턴을 재사용할 수 있다.
...
12. `supabase/skill_tags_global_unique.sql` — 능력 태그 이름을 팀 구분 없이 전역 유일하게 변경(기존 중복 태그 자동 병합 포함) (미실행)
